...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', "REPORTE DE TICKETS DE: $fullname")
...
$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(20);	
	...
	$sql="SELECT * FROM tickets where client_id=$client_id order by created_at";
	...
	while ($row=mysqli_fetch_array($query)){
		...
		if($date_atendid=="0000-00-00 00:00:00"){
			...
			// Agregar datos
			$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue($a, $business)
            ->setCellValue($b, $number_ticket)
            ->setCellValue($c, $name)
            ->setCellValue($d, $email)
			->setCellValue($e, $name_status)
